...
import chap11.model.FileAccess;
...
public class StudentController {
	...
	public StudentController() throws IOException {
		...
		fa = new FileAccess(); // 파일접근 객체 생성 
	}
	
	// 학생정보 저장 메서드 
	public void insertCall(int size) throws IOException{
		...
		for (int i=0; i < size; i++){ // 학생수 만큼 반복 
			...
			fa.file_insert(list);  // 저장 메서드 호출
	}// method end 
	
	// 학생정보 조회 메서드 
	public List<Student> selectCall(int size) throws IOException{		
		List<Student> list = fa.file_select(size);		
		return list;		
	}// method end  
